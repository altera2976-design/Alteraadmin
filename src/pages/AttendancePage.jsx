...
export default function AttendancePage() {
  ...
  const handleSubmitReview = async () => {
    ...
    setReviewFeedback('');
    try {
      const res = await api.patch(`/attendance/review/${selectedRecord.attendanceId}`, {
  ...
  const filteredDailyList = dailyList.filter((emp) => {
    ...
    const matchesStatus =
      ...
      (statusFilter === 'NEEDS_REVIEW' && emp.verificationStatus === 'REVIEW_REQUIRED');
      ...
      <div style={styles.heroBanner}>
        ...
        <div style={styles.heroContent}>
          <div>
            ...
            <h2 style={styles.heroTitle}>Staff Attendance & Geofence Verification</h2>
            <p style={styles.heroSub}>
              Real-time facial selfie verification, GPS location tracking, working hours audit, and automated office geofencing.
            </p>
          </div>
        ...
        {stats && (
          <div style={styles.kpiGrid}>
            ...
            <div style={styles.kpiCard}>
              <div style={{ ...styles.kpiIcon, background: 'rgba(13, 148, 136, 0.2)' }}>✅</div>
              <div>
                <div style={{ ...styles.kpiNum, color: '#10B981' }}>{stats.present}</div>
                <div style={styles.kpiLabel}>Present On-Duty</div>
              </div>
            </div>
            <div style={styles.kpiCard}>
              <div style={{ ...styles.kpiIcon, background: 'rgba(217, 119, 6, 0.2)' }}>⏰</div>
              <div>
                <div style={{ ...styles.kpiNum, color: '#F59E0B' }}>{stats.late}</div>
                <div style={styles.kpiLabel}>Late Arrivals</div>
              </div>
            </div>
            <div style={styles.kpiCard}>
              <div style={{ ...styles.kpiIcon, background: 'rgba(220, 38, 38, 0.2)' }}>❌</div>
              <div>
                <div style={{ ...styles.kpiNum, color: '#F87171' }}>{stats.absent}</div>
                <div style={styles.kpiLabel}>Absent / Unmarked</div>
              </div>
            </div>
            <div style={styles.kpiCard}>
              <div style={{ ...styles.kpiIcon, background: 'rgba(197, 160, 89, 0.2)' }}>📊</div>
              <div>
                <div style={{ ...styles.kpiNum, color: '#C5A059' }}>{stats.attendancePercentage}%</div>
      ...
      <div style={styles.tabContainer}>
        <div style={styles.tabBar}>
          ...
          >
            ...
            <span style={activeTab === 'daily' ? styles.tabCountActive : styles.tabCount}>
              {dailyList.length}
            </span>
          </button>
          <button
          ...
          <button
            style={activeTab === 'geofence' ? styles.tabBtnActive : styles.tabBtn}
        ...
        <div>
          ...
          <div className="card" style={styles.filterCard}>
            ...
            <div style={{ fontSize: 13, color: '#6F6960', fontWeight: 600 }}>
              Showing {filteredDailyList.length} of {dailyList.length} staff members
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="card" style={styles.emptyCard}>
              ...
              <h3 style={{ margin: '0 0 6px', fontSize: 17, fontWeight: 700 }}>No Attendance Records Found</h3>
              <p style={{ color: '#6F6960', fontSize: 13, margin: 0 }}>
                No records matched your search on {selectedDate}. Staff can submit their selfie check-ins using the mobile app.
              </p>
            </div>
          ) : (
                ...
                <table style={styles.table}>
                  ...
                  <tbody>
                    {filteredDailyList.map((emp) => {
                      ...
                      const checkOutSelfieUrl = getSelfieUrl(emp.checkOutSelfie);
                        ...
                        <tr key={emp._id} style={styles.trRow}>
                          ...
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <div>
                                ...
                                <div style={styles.employeeIdBadge}>{emp.employeeId || 'EMP-ID'}</div>
                              </div>
                            </div>
                          </td>
                            ...
                            {hasCheckInSelfie ? (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <img
                                  ...
                                  onClick={() => setZoomedImage(checkInSelfieUrl)}
                                ...
                                <div>
                                  ...
                                  <span style={styles.clickToZoom}>View Selfie 🔍</span>
                                </div>
                              </div>
                            ) : (
                              <div style={styles.noSelfiePlaceholder}>
                                <span>—</span>
                                <span style={{ fontSize: 11, color: '#9E978C' }}>No Check-in</span>
                              </div>
                            )}
                          </td>
                            ...
                            {hasCheckOutSelfie ? (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <img
                                  ...
                                  style={styles.selfieThumb}
                                  onClick={() => setZoomedImage(checkOutSelfieUrl)}
                                  title="Click to view full image"
                                />
                                <div>
                                  <div style={styles.timeTag}>
                              ...
                              <div style={styles.noSelfiePlaceholder}>
                                <span>—</span>
                                <span style={{ fontSize: 11, color: '#9E978C' }}>
                                  {hasCheckInSelfie ? 'On Duty' : 'No Check-out'}
                                </span>
                              </div>
                            )}
                          </td>
                          ...
                          <td>
                            <div style={styles.hoursBadge}>
                              {emp.totalHours ? `${emp.totalHours} hrs` : emp.checkInTime ? 'Active' : '—'}
                            </div>
                          </td>
                          ...
                          <td>
                            {emp.geofenceStatus ? (
                              <div>
                                <span style={getGeofenceStatusStyle(emp.geofenceStatus)}>
                                  {emp.geofenceStatus === 'INSIDE' ? '📍 Inside Office' : emp.geofenceStatus === 'DISABLED' ? '🌐 Remote' : '⚠️ Outside'}
                                </span>
                                {emp.checkInLocation?.distance !== undefined && (
                                  <div style={{ fontSize: 11, color: '#6F6960', marginTop: 2 }}>
                                    {Math.round(emp.checkInLocation.distance)}m from HQ
                                  </div>
                                )}
                              </div>
                            ) : (
                              <span style={{ color: '#9E978C', fontSize: 12 }}>—</span>
                            )}
                          </td>
                          ...
                          <td>
                            {emp.attendanceId ? (
                              <span style={getVerificationBadgeStyle(emp.verificationStatus)}>
                                {emp.verificationStatus === 'VERIFIED' ? '✓ Verified' : emp.verificationStatus === 'FAILED' ? '✕ Failed' : '⚠️ Needs Review'}
                              </span>
                            ) : (
                              <span style={{ color: '#9E978C', fontSize: 12 }}>Unmarked</span>
                            )}
                          </td>
                          ...
                          <td style={{ textAlign: 'right' }}>
                            {emp.attendanceId ? (
                              ...
                              <span style={{ color: '#C8CBD4', fontSize: 12 }}>—</span>
                            )}
                          </td>
                        </tr>
                      );
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
              <div className="card" style={styles.monthlyKpiCard}>
                ...
                <div style={{ ...styles.monthlyKpiVal, color: '#10B981' }}>{monthlyStats.present}</div>
                <div style={styles.monthlyKpiSub}>Full working days recorded</div>
              </div>
              <div className="card" style={styles.monthlyKpiCard}>
                <div style={{ ...styles.monthlyKpiIcon, background: 'rgba(217, 119, 6, 0.1)' }}>⏰</div>
                <div style={styles.monthlyKpiLabel}>Total Late Days</div>
                <div style={{ ...styles.monthlyKpiVal, color: '#F59E0B' }}>{monthlyStats.late}</div>
                <div style={styles.monthlyKpiSub}>After standard check-in buffer</div>
              </div>
              <div className="card" style={styles.monthlyKpiCard}>
                <div style={{ ...styles.monthlyKpiIcon, background: 'rgba(159, 11, 34, 0.1)' }}>🌗</div>
                <div style={styles.monthlyKpiLabel}>Half Day Sessions</div>
                <div style={{ ...styles.monthlyKpiVal, color: '#9F0B22' }}>{monthlyStats.halfDay}</div>
                <div style={styles.monthlyKpiSub}>Under 4 hours logged</div>
              </div>
              <div className="card" style={styles.monthlyKpiCard}>
                <div style={{ ...styles.monthlyKpiIcon, background: 'rgba(220, 38, 38, 0.1)' }}>❌</div>
                <div style={styles.monthlyKpiLabel}>Total Absences</div>
                <div style={{ ...styles.monthlyKpiVal, color: '#DC2626' }}>{monthlyStats.absent}</div>
      ...
      {activeTab === 'geofence' && (
        <div style={{ maxWidth: 760 }}>
          <div className="card" style={{ padding: 28 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18, borderBottom: '1px solid #E8E3DA', paddingBottom: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(159, 11, 34, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>
                📍
              </div>
              <div>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, fontFamily: "'Outfit', sans-serif" }}>
                  Altera Studio Geofence Boundaries
                </h3>
                <p style={{ margin: '3px 0 0', fontSize: 13, color: '#6F6960' }}>
                  Define the authorized GPS perimeter for staff selfie attendance check-in.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveGeofence} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label className="form-label">Geofence Verification Mode</label>
                <select
                  className="form-select"
                  value={geofenceConfig.geofenceMode}
                  onChange={(e) => setGeofenceConfig({ ...geofenceConfig, geofenceMode: e.target.value })}
                >
                  <option value="REQUIRED">Mandatory (Block check-in if employee is outside allowed radius)</option>
                  <option value="OPTIONAL">Audit Mode (Allow check-in, but flag as Outside for Admin review)</option>
                  <option value="DISABLED">Disabled (Allow attendance from anywhere)</option>
                </select>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setGeofenceConfig({ ...geofenceConfig, officeName: e.target.value })}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setGeofenceConfig({ ...geofenceConfig, radius: Number(e.target.value) })}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setGeofenceConfig({ ...geofenceConfig, officeAddress: e.target.value })}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setGeofenceConfig({ ...geofenceConfig, latitude: Number(e.target.value) })}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setGeofenceConfig({ ...geofenceConfig, longitude: Number(e.target.value) })}
                  />
                </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
                <button type="submit" className="btn btn-primary" disabled={isSavingGeofence}>
                  {isSavingGeofence ? 'Saving Settings...' : 'Save Geofence Configuration'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={styles.modalBackdrop}>
          <div style={styles.modalCard}>
            <div style={styles.modalHeader}>
              ...
              <button onClick={() => setSelectedRecord(null)} style={styles.closeBtn}>✕</button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginTop: 20 }}>
              ...
              <div style={styles.selfieCard}>
                <div style={styles.selfieCardHeader}>
                  <span style={{ fontWeight: 700, fontSize: 13, color: '#1A1A1E' }}>Check-In Selfie</span>
                ...
                {selectedRecord.checkInSelfie ? (
                  <img
                    ...
                    onClick={() => setZoomedImage(getSelfieUrl(selectedRecord.checkInSelfie))}
                  />
                ) : (
                  <div style={styles.selfieEmptyBox}>No Check-In Selfie Recorded</div>
                )}
                <div style={styles.selfieLocationInfo}>
                  <div>📍 <strong>Address:</strong> {selectedRecord.checkInLocation?.address || 'GPS coordinates logged'}</div>
                  {selectedRecord.checkInLocation?.distance !== undefined && (
                    <div>📏 <strong>Distance:</strong> {Math.round(selectedRecord.checkInLocation.distance)}m from HQ</div>
                  )}
                </div>
              </div>
              ...
              <div style={styles.selfieCard}>
                <div style={styles.selfieCardHeader}>
                  <span style={{ fontWeight: 700, fontSize: 13, color: '#1A1A1E' }}>Check-Out Selfie</span>
                  <span style={styles.timePill}>
                    {selectedRecord.checkOutTime ? new Date(selectedRecord.checkOutTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'On Duty'}
                  </span>
                </div>
                {selectedRecord.checkOutSelfie ? (
                  <img
                    ...
                    alt="Check-out Selfie"
                    style={styles.selfieModalImg}
                    onClick={() => setZoomedImage(getSelfieUrl(selectedRecord.checkOutSelfie))}
                  />
                ) : (
                  <div style={styles.selfieEmptyBox}>
                    {selectedRecord.checkInSelfie ? 'Employee has not checked out yet' : 'No Check-out Selfie Recorded'}
                  </div>
                )}
                <div style={styles.selfieLocationInfo}>
                  <div>📍 <strong>Address:</strong> {selectedRecord.checkOutLocation?.address || '—'}</div>
                  <div>⏳ <strong>Total Session:</strong> {selectedRecord.totalHours ? `${selectedRecord.totalHours} hrs` : 'In Progress'}</div>
                </div>
              </div>
            </div>
            ...
            <div style={styles.reviewFormSection}>
              <h4 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 700, color: '#1A1A1E' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div>
                  ...
                  >
                    <option value="VERIFIED">VERIFIED (Face & Location Approved)</option>
                    <option value="REVIEW_REQUIRED">REVIEW_REQUIRED (Flag for Investigation)</option>
                    <option value="FAILED">FAILED (Spoof / Proxy Check-in Attempt)</option>
                  </select>
                </div>
              ...
              <div style={{ marginTop: 12 }}>
                <label className="form-label">Reviewer Notes (Visible in Audit Trail)</label>
                <input
                  type="text"
                  className="form-input"
      ...
      {zoomedImage && (
        <div style={styles.zoomBackdrop} onClick={() => setZoomedImage(null)}>
          <div style={styles.zoomContent} onClick={(e) => e.stopPropagation()}>
            <img src={zoomedImage} alt="Enlarged Selfie" style={styles.zoomImg} />
            <button style={styles.zoomCloseBtn} onClick={() => setZoomedImage(null)}>✕ Close</button>
          </div>
        </div>
      )}
    </AdminLayout>
...
function getAttendanceStatusStyle(status) {
  ...
  switch (status) {
    ...
    case 'LATE':
      return { ...base, background: '#FFFBEB', color: '#92400E', border: '1px solid #FDE68A' };
    case 'HALF_DAY':
    ...
    default:
      return { ...base, background: '#F3EFEA', color: '#6F6960', border: '1px solid #E8E3DA' };
  }
}
...
function getGeofenceStatusStyle(status) {
  ...
  switch (status) {
    case 'INSIDE':
      return { ...base, background: '#F0FDF4', color: '#166534', border: '1px solid #BBF7D0' };
    case 'OUTSIDE':
      return { ...base, background: '#FEF2F2', color: '#991B1B', border: '1px solid #FECACA' };
    default:
...
const styles = {
  heroBanner: {
    background: 'linear-gradient(135deg, #121316 0%, #291217 55%, #18191E 100%)',
    borderRadius: 18,
  ...
  heroGlow: {
    ...
    height: 240,
    borderRadius: '50%',
    background: 'radial-gradient(circle, rgba(159, 11, 34, 0.35) 0%, transparent 70%)',
    pointerEvents: 'none',
  },
  heroContent: {
    display: 'flex',
